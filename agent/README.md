...
| 返回 `NotSupported` | Plasma 6 Wayland | **退化为锁屏状态**：已锁屏 → 用锁屏时长（`GetActiveTime`）视为离开；未锁屏 → `0`（按在用处理） |
...
- 只有 `0 <= idle < 86400` 的返回值才被采信，超出区间按 `0` 上报并计入异常计数；
  连续 5 次异常打一条 WARN，之后降频提醒，**不会中断上报**。
- 若 `idle` 连续 20 个周期恒为 0，会额外提示一次「空闲检测很可能不可用」——
  这正是 KDE 论坛上报告过的恒 0 现象。
- `GetSessionIdleTime` 规范单位是**秒**，Agent 按秒解读，不做单位换算猜测。
  若某实现返回毫秒，数值会迅速越过 86400 而被判为异常并告警（宁可告警也不上报错数据）。
